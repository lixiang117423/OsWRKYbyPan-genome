...
df <- pac4xiang::fasta2df("results/step4_seq_related/all.transcript.20220306.fasta")

# Paul J. Rushton
df_new <- df %>%
  dplyr::mutate(
    #first_char = stringr::str_extract(seq, "\\w{1}(WRKYGQK)"),
    first_char = stringr::str_extract(seq, "(WRKYG)\\w{1}(K)"),
...
data.table::fwrite(df_new, file = "results/step4_seq_related/seq_subgroup_chenxujun_all_transceipt.txt", row.names = FALSE)
...
data.table::fwrite(df_new, file = "results/step4_seq_related/seq_subgroup_CH_type_all_transcript.txt", row.names = FALSE)


# 提取基因名称
df_new %>% 
  dplyr::mutate(gene = stringr::str_sub(id,2, nchar(id) - 7)) %>% 
  dplyr::mutate(new.seq = paste0(first_char, sec_char, third_char))-> df_new.2
